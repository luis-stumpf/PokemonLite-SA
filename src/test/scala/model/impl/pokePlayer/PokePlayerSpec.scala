...
class PokePlayerSpec extends AnyWordSpec {
  "PokePlayerSpec" should {
    val pokeList1 = PokePack( List( Pokemon( Glurak ), Pokemon( Glurak ) ) )
    val pokeList2 =
      PokePack( List( Pokemon( Brutalanda ), Pokemon( Simsala, 0, true ) ) )
    val pokeList3 =
      PokePack( List( Pokemon( Simsala ), Pokemon( Simsala ) ) )
    val pokeListDead = PokePack(
      List( Pokemon( Simsala, 0, true ), Pokemon( Glurak, 0, true ) )
    )
    ...
    "check Pokemons" in {
      p1.pokemons should be( PokePack.apply( List() ) )
      p2.pokemons should be(
        PokePack(
          List( Pokemon( Brutalanda ), Pokemon( Simsala, 0, true ) ),
          2
        )
      )
