  ...
  override def update( message: String ): Unit =
    ...
    topPane.children = new VBox( 10 ) {
      ...
      var undo: Button = new Button( "<-" ) {
        onAction = _ => controller.doAndPublish( controller.undoMove )
      }
      var redo: Button = new Button( "->" ) {
        onAction = _ => controller.doAndPublish( controller.redoMove )
      }
      var load: Button = new Button( "load" ) {
        onAction = _ => controller.doAndPublish( controller.load )
      }
      var save: Button = new Button( "save" ) {
        onAction = _ => controller.doAndPublish( controller.save )
      }
