...
case class PlayerPokemonPane( controller: ControllerInterface )
    extends GridPane {
  ...
  var list = ( 1 to Game.maxPokePackSize ).map[ComboBox[String]]( p =>
    new ComboBox[String]( PokemonType.values.map( "" + _.name ) ) {
      minWidth = 200
      minHeight = 40
      margin = Insets( 20 )
    }
