...
case class HealthBar( controller: ControllerInterface, turn: Int )
    extends StackPane() {
  ...
  def calcHealthBar: Double =
    ...
    var res = 0
    if (turn == 1) then
      currentHealth = controller.game.player1
        .map(
          _.pokemons.contents
            .apply( controller.game.player1.get.currentPoke )
            .map( _.getHp )
        ...
        .getOrElse( 0 )
      maxHealth = controller.game.player1
        .map(
          _.pokemons.contents
            .apply( controller.game.player1.get.currentPoke )
            .map( _.pType.hp )
      ...
      currentHealth = controller.game.player2
        .map(
          _.pokemons.contents
            .apply( controller.game.player2.get.currentPoke )
            .map( _.getHp )
        ...
        .getOrElse( 0 )
      maxHealth = controller.game.player2
        .map(
          _.pokemons.contents
            .apply( controller.game.player2.get.currentPoke )
            .map( _.pType.hp )
