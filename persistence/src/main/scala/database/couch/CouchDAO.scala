...
object CouchDAO extends DAOInterface {
  ...
  def createDatabase(): Future[HttpResponse] = {
    ...
    val request = Put( uri ).withHeaders(
  ...
  override def save( game: GameInterface ): Try[Unit] = Try {
    ...
    val request = Put(
      uri,
      HttpEntity( ContentTypes.`application/json`, gameJson.toString )
    )
      .withHeaders(
        headers.Authorization(
          headers.BasicHttpCredentials( config.username, config.password )
  ...
  override def load( gameId: Option[Int] = None ): Try[GameInterface] = Try {
    ...
    val uri = s"http://${config.host}:${config.port}/$databaseName/$id"
    val request = Get( uri ).withHeaders(
      headers.Authorization(
        headers.BasicHttpCredentials( config.username, config.password )
      )
    )
    ...
    val result = responseFuture.flatMap { response =>
      if (response.status.isSuccess()) {
        val jsonFuture =
          Unmarshal( response.entity ).to[String].map( Json.parse )
      ...
      responseFuture.onComplete {
        ...
        case Success( response ) =>
          Unmarshal( response.entity )
            ...
            .foreach( entity =>
              println(
                s"Failed to update game $id: ${response.status}, $entity"
              )
            )
        case Failure( exception ) =>
  ...
  override def delete( gameId: Option[Int] ): Try[Unit] = Try {
    ...
    val fetchRequest = Get( uri ).withHeaders(
      headers.Authorization(
        headers.BasicHttpCredentials( config.username, config.password )
      )
    )

    val fetchResponseFuture: Future[HttpResponse] =
      Http().singleRequest( fetchRequest )
    ...
    val deleteRequest = Delete( deleteUri ).withHeaders(
      headers.Authorization(
        headers.BasicHttpCredentials( config.username, config.password )
      )
    )

    val deleteResponseFuture: Future[HttpResponse] =
      Http().singleRequest( deleteRequest )
