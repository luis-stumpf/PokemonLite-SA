...
object Pokemon {
  def apply( pType: PokemonType ): Pokemon =
    Pokemon( pType = pType, hp = pType.hp )
...
case class Pokemon( pType: PokemonType, hp: Int, isDead: Boolean = false ) {
  ...
  def increaseHP( amount: Double ): Pokemon =
    if pType.hp <= ( hp + amount ) then copy( hp = pType.hp )
    else copy( hp = ( hp + amount ).toInt, isDead = false )

  def reduceHP( amount: Double ): Pokemon =

    val updatedHealth = ( hp - amount ).toInt

    if updatedHealth <= 0 then copy( hp = updatedHealth, isDead = true )
    else copy( hp = updatedHealth )

  def damageOf( attackNumber: Int ): Int =
    pType.attacks.apply( attackNumber ).damage

  def getHp: Int = hp

  override def toString: String =
    if hp <= 0 then pType.name + " is dead" else pType.name + " HP: " + hp
}
...
enum PokemonType(
  ...
  case Simsala
      extends PokemonType(
        ...
        attacks = List(
          Attack( "Konfusion", 10 ),
          Attack( "Psychoklinge", 15 ),
          Attack( "Psychokinese", 30 ),
          Attack( "Eishieb", 15 )
        ),
  ...
  case Brutalanda
      extends PokemonType(
        ...
        attacks = List(
          Attack( "Fliegen", 20 ),
          Attack( "Drachenklaue", 30 ),
          Attack( "Glut", 10 ),
          Attack( "Flammenwurf", 35 )
        ),
        pokemonArt = PokemonArt.Feuer
      )
  ...
  case Bisaflor
      extends PokemonType(
        ...
        hp = 180,
        attacks = List(
          Attack( "Rasierblatt", 20 ),
          Attack( "Tackle", 10 ),
          Attack( "Solarstrahl", 30 ),
          Attack( "Matschbombe", 15 )
        ),
  ...
  case Turtok
      extends PokemonType(
        ...
        hp = 130,
        attacks = List(
          Attack( "Aquaknarre", 20 ),
          Attack( "Biss", 15 ),
          Attack( "Hydropumpe", 40 ),
          Attack( "Matschbombe", 15 )
        ),
